...
void send_response(CANMsg *self, int unused);
void update_neighbors(Melody *self, int unused);
...
void receiver(App *self, int unused) {
	...
	if(msg.msgId == REQUEST && msg.nodeId == 15) {
		...
		SYNC(&rmsg, send_response, 0);
	}
	else if(msg.msgId == RESPONSE) {
		SCI_WRITE(&sci0, "Recevied response message!\n");
		update_neighbors(&m, 0);
	}
	else {
		SCI_WRITE(&sci0, "Fail\n");
	}
