...
//#define	__CAN_TxAck   // Note: enable single transmission mode
...
// Return value:
//   0: successful copy
//   1: no message pending in software buffer
//
...
//
// if __CAN_TxAck: single transmission mode, can_send() will wait for message error or acknowledgement
// else: automatic retransmission mode, can_send() will not wait for message error or acknowledgement
//
// Return value:
//   0: successful transmission
//      if __CAN_TxAck: acknowledgement of correct reception
//      else: message sent without waiting for acknowledgement of correct reception, will retransmit automatically
//   1: transmission failed
//      if __CAN_TxAck: no acknowledgement, transmit error counter is read and stored in self->tecl
//      else: transmit buffer full, 3 pending messages
//
int can_send(Can *self, CANMsg *msg){
	...
	self->tecl = CAN_GetLSBTransmitErrorCounter(canport);
...
//	DUMP(">> ");
	...
	if (self->tecl > 96) {
		if (self->tecl > 127)
			DUMP("CAN Error Passive (TEC>127)\n\r");
		else
			DUMP("CAN Error Warning (TEC>96)\n\r");
	}
#endif
